...
pearydata clicpix2_frameDecoder::getZerosuppressedFrame() {
  ...
  for(auto r = 0u; r < clicpix2_frameDecoder::CLICPIX2_ROW; ++r) {
    for(auto c = 0u; c < clicpix2_frameDecoder::CLICPIX2_COL; ++c) {
...
void clicpix2_frameDecoder::decodeHeader(const clicpix2_frameDecoder::WORD_TYPE word) {
  if(word.is_control_ != 0) {
    throw DataException("Packet header should be a regular data word: " + to_hex_string(word.word_));
  }

  rcr = (word.word_ >> 6) & 0x3;

  if(rcr == 0) {
    throw DataException("Unsupported RCR in packet header");
  }

  firstColumn = word.word_ & 0x1F;
}
...
#define DC_COUNTER_INIT (2 * (CLICPIX2_ROW * CLICPIX2_PIXEL_SIZE + CLICPIX2_ROW / CLICPIX2_SUPERPIXEL_SIZE) + 1)
  ...
  do {
    ...
    unraveDC(pixels_dc, dc_counter, sp_counter, row_index, row_slice, word.word_);
  ...
  for(unsigned int r = 0; r < 256; ++r)
    for(unsigned int c = 0; c < static_cast<unsigned int>(1 << rcr); c++) {
      switch(r % 4) {
      case 0:
        matrix[r / 2][c * 2 * 64 / static_cast<unsigned int>(1 << rcr) + firstColumn * 2u] = pixels_dc[r][c];
        break; // left column
      case 1:
        matrix[r / 2][c * 2 * 64 / static_cast<unsigned int>(1 << rcr) + firstColumn * 2u + 1] = pixels_dc[r][c];
        break; // right column
      case 2:
        matrix[r / 2][c * 2 * 64 / static_cast<unsigned int>(1 << rcr) + firstColumn * 2u + 1] = pixels_dc[r][c];
        break; // right column
  ...
  if(dc_counter < static_cast<int>(2 * (CLICPIX2_ROW * CLICPIX2_PIXEL_SIZE + CLICPIX2_ROW / CLICPIX2_SUPERPIXEL_SIZE))) {
    ...
    if(sp_counter == 0) {
      if(data ||                               // not empty super-pixel
        ...
        dc_counter += static_cast<int>(CLICPIX2_SUPERPIXEL_SIZE * CLICPIX2_PIXEL_SIZE) + 1;
      }
    }
    ...
    // pixel bit
      ...
      if(data ||                     // not empty pixel
        ...
        pixels_dc[row_index++][col_index].setLatches(0x00);
        dc_counter += static_cast<int>(CLICPIX2_PIXEL_SIZE);
        sp_counter += static_cast<int>(CLICPIX2_PIXEL_SIZE);
      }
    }
  ...
  for(auto r = 0u; r < clicpix2_frameDecoder::CLICPIX2_ROW; ++r) {
    for(auto c = 0u; c < clicpix2_frameDecoder::CLICPIX2_COL; ++c) {
      ...
      if(counter_config[r][c]) {
        ...
        matrix[r][c].SetTOA(LFSR::LUT8(static_cast<uint8_t>(matrix[r][c].GetLatches() & 0xff)));
      }
    }
  }
}
